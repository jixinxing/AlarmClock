...
package cn.just.alarmclock.service;
...
/**
 * 允许别的Activity打断正在响铃的铃声，播放其他的铃声，例如，闹钟响的时候来电话了。
 */
public class AlarmKlaxon extends Service {
	...
	private AlarmClock mCurrentAlarm;
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case KILLER:
				...
				sendKillBroadcast((AlarmClock) msg.obj);
				stopSelf();
				break;
			case FOCUSCHANGE:
				switch (msg.arg1) {
	...
	public void onCreate() {
		mVibrator = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE);//振动
		mAudioManager = (AudioManager) getSystemService(AUDIO_SERVICE);//声音
		mTelephonyManager = (TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE);//电话服务
	...
	private void sendKillBroadcast(AlarmClock alarm) {
	...
	private void play(AlarmClock alarm) {
		...
		if (!alarm.silent) {
			...
			mMediaPlayer.setOnErrorListener(new OnErrorListener() {
				public boolean onError(MediaPlayer mp, int what, int extra) {
					Log.v("tag", "Error occurred while playing audio.");
					mp.stop();
					mp.release();
					mMediaPlayer = null;
					return true;
			...
			try {
				...
				try {
					...
					Log.v("tag", "Failed to play fallback ringtone"
							+ ex2);
				}
			}
		}
	...
	private void enableKiller(AlarmClock alarm) {
